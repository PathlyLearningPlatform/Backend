...
import { mockActivityRepo, makeQuiz, makeArticle, TEST_IDS } from '../../common';
...
describe('AddQuestionHandler', () => {
	...
	it('throws ActivityNotFoundException when quiz not found', async () => {
		...
		await expect(
			handler.execute({ quizId: TEST_IDS.QUIZ_ID, content: 'Q', correctAnswer: 'A' }),
	...
	it('throws ActivityNotFoundException when activity is not a Quiz', async () => {
		...
		await expect(
			handler.execute({ quizId: TEST_IDS.ARTICLE_ID, content: 'Q', correctAnswer: 'A' }),
		).rejects.toThrow(ActivityNotFoundException);
	});
});
